...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def initialize(dataset_folder, M, N, alpha, L, min_images_per_class, filename):
        ...
        ###--------------------------------------------------------------
        for index, class_per_group in enumerate(classes_per_group):
            with open("output{}.csv".format(index), mode="w", newline="", encoding="utf-8") as file:
                writer = csv.writer(file)
                for class_id in class_per_group:
                    images_path = images_per_class.get(class_id,[])
                    for image_path in images_path:
                        writer.writerow([class_id,image_path])
            print("finished writing group{}!".format(index))
        ###--------------------------------------------------------------
